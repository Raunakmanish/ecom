...
if(isset($_POST['submit'])){
		if(!empty($_SESSION['fcart'])){
		foreach($_POST['quantity'] as $key => $val){
			if($val==0){
				unset($_SESSION['fcart'][$key]);
			}else{
				$_SESSION['fcart'][$key]['quantity']=$val;
...
mysqli_query($con,"insert into medicineorders(userId,productId,quantity) values('".$_SESSION['fid']."','$qty','$val34')");
...
// code for billing address updation
	if(isset($_POST['update']))
	{
		...
		$query=mysqli_query($con,"update users set billingAddress='$baddress',billingState='$bstate',billingCity='$bcity',billingPincode='$bpincode' where id='".$_SESSION['fid']."'");
		if($query)
		{
...
// code for Shipping address updation
	if(isset($_POST['shipupdate']))
	{
		...
		$query=mysqli_query($con,"update users set shippingAddress='$saddress',shippingState='$sstate',shippingCity='$scity',shippingPincode='$spincode' where id='".$_SESSION['fid']."'");
		if($query)
		{
...
if(!empty($_SESSION['fcart'])){
 ...
 <?php
 $pdtid=array();
    $sql = "SELECT * FROM medicineproducts WHERE id IN(";
			foreach($_SESSION['fcart'] as $id => $value){
			$sql .=$id. ",";
			}
			...
			while($row = mysqli_fetch_array($query)){
				$quantity=$_SESSION['fcart'][$row['id']]['quantity'];
				$subtotal= $_SESSION['fcart'][$row['id']]['quantity']*$row['productPrice']+$row['shippingCharge'];
				$totalprice += $subtotal;
				...
				<tr>
					...
					<td class="cart-image">
						<a class="entry-thumbnail" href="detail.html">
						    <img src="../mainadmin/admin/productimages/<?php echo $row['id'];?>/<?php echo $row['productImage1'];?>" alt="" width="114" height="146">
						</a>
					</td>
...
?>
					...
					<td class="cart-product-quantity">
						<div class="quant-input">
				             ...
				             <input type="text" value="<?php echo $_SESSION['fcart'][$row['id']]['quantity']; ?>" name="quantity[<?php echo $row['id']; ?>]">
...
<td class="cart-product-sub-total"><span class="cart-sub-total-price"><?php echo "Rs"." ".$row['shippingCharge']; ?>.00</span></td>
					...
					<td class="cart-product-grand-total"><span class="cart-grand-total-price"><?php echo ($_SESSION['fcart'][$row['id']]['quantity']*$row['productPrice']+$row['shippingCharge']); ?>.00</span></td>
				</tr>
...
<?php
$query=mysqli_query($con,"select * from users where id='".$_SESSION['fid']."'");
while($row=mysqli_fetch_array($query))
{
?>
